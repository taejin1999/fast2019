...
\centering
\begin{pgfpicture}
\pgftext{\pgfimage[width=15cm,height=4cm]{scratch.png}}
\end{pgfpicture}
...
\textcolor{red}{To quickly assign incoming data to a proper stream ID, we add
...
\begin{comment}
Maintaining lifetime information of all the PCs including one-time write PC
is unnecessary and inefficient because our goal is to identify the dominant PCs.
In order to filter PCs with small impact, 
every PCs have training 
manage PCs with larger number 
of writes than threshold value set nubmer of writes
to be managed, for example, 100 times.
\end{comment}
