...
the long-lived data of {\it $PC_j$} are moved to a different streamof {\it $S_k'$}
during GC.  
The stream {\it $S_k'$} prevents the long-lived data of the stream {\it $S_k$} 
...
Using internal streams, \textsf{\small PCStream} can effectively overcome
the diffcult stream allocation problem with program
contexts with large variances without consuming valuable normal
streams.
