...
streams'' interchangeably.}, with which data separation decisions {\it on the
...
In \textsf{\small PCStream}, we employ a write program context\endnote{In this
paper, since we are interested in write-related system calls such as write() in
...
IOPS by \error{28\%} while reducing the average WAF by \error{49\%.}
...
\note{In Section 2, we review existing stream management techniques.}
