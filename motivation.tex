...
{\color{blue}
when no apparent {\it locality} on LBA accesses exist in applications.  
...
\begin{figure}[t]
	...
	\hfill
	\vspace{2pt}
	\subfloat[Lifetime patterns over time for the chunk A]{\includegraphics[width=0.21\textwidth]{figure/lifetime_in_chunk}} % data from 0/03031641
	\hspace{10pt}
	\subfloat[Lifetime patterns over time for the chunk B]{\includegraphics[width=0.21\textwidth]{figure/lifetime_in_chunk3}}
...
Figs.~\ref{fig:lba_lifetime}(c) and ~\ref{fig:lba_lifetime}(d) shows
...
{\color{blue}
In practice, LBA-based data separations works well only when the LBA access 
locality is obvious in I/O activities such as metadata file or log file updates.  
}
...
\begin{figure}[t]
	...
	\includegraphics[width=0.8\linewidth]{figure/stream_perf}
	%\vspace{-9pt}
...
{\color{blue}
For example, as shown in Fig.~\ref{fig:stream_perf},
...
In the example, we ran RocksDB, which has about 4$\sim$5 I/O activities 
with distinct data lifetime characteristics,
on the Samsung PM963 multi-streamed SSD that can support up to 8 streams.
We modified RocksDB implementation to allocate I/O activites to a stream.
The IOPS was measured by \texttt{db\_bench} benchmark and saturated after 
RocksDB can utilize enough number streams (more than 4) for
data from most I/O activities to be allocated to separate streams.
}
